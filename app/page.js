...
Favorite food: ${form.food || "anything local"}
Needs: ${form.needs || "standard travel needs"}`;
  ...
  const reset = () => {
    setStep("input");
    setResult(null);
    setForm({ city:"", age:"", with:"", interests:[], food:"", needs:"" });
    setError("");
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#faf7f2", fontFamily:"'Lora', Georgia, serif", color:"#1c1917", padding:"0 0 80px" }}>
      ...
      <div style={{ background:"linear-gradient(160deg,#292524,#1c1917)", padding:"48px 24px 56px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", bottom:-60, left:-30, width:160, height:160, borderRadius:"50%", background:"rgba(251,146,60,0.06)" }} />
        <div style={{ display:"inline-block", background:"rgba(245,158,11,0.15)", border:"1px solid rgba(245,158,11,0.3)", borderRadius:20, padding:"4px 16px", marginBottom:20, fontSize:11, color:"#f59e0b", letterSpacing:2, textTransform:"uppercase", fontFamily:"'DM Sans',sans-serif" }}>
          ✦ Your Day, Your Story
        </div>
        <h1 style={{ fontSize:"clamp(28px,6vw,52px)", fontWeight:700, margin:"0 0 14px", color:"#faf7f2", lineHeight:1.2, fontStyle:"italic" }}>
          Plan Your Perfect Day
        </h1>
        <p style={{ color:"#a8a29e", fontSize:16, margin:"0 auto", fontFamily:"'DM Sans',sans-serif", maxWidth:400, lineHeight:1.6 }}>
          Tell us about yourself — we'll write your day as a story
        </p>
      </div>
          ...
          <div className="fade">
            <div style={{ background:"#fff", borderRadius:20, padding:28, boxShadow:"0 4px 40px rgba(0,0,0,0.08)", border:"1px solid #e7e5e4" }}>
              ...
              <div style={{ marginBottom:20 }}>
                ...
                <input value={form.city} onChange={e => setForm(f=>({...f,city:e.target.value}))} placeholder="Oslo, Paris, Tokyo..." style={{ width:"100%", border:"1.5px solid #e7e5e4", borderRadius:12, padding:"13px 16px", fontSize:16, color:"#1c1917", outline:"none", background:"#faf7f2" }} onFocus={e=>e.target.style.borderColor="#f59e0b"} onBlur={e=>e.target.style.borderColor="#e7e5e4"} />
              </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:20 }}>
                <div>
                  ...
                  <input value={form.age} onChange={e=>setForm(f=>({...f,age:e.target.value}))} placeholder="25" type="number" style={{ width:"100%", border:"1.5px solid #e7e5e4", borderRadius:12, padding:"13px 16px", fontSize:15, color:"#1c1917", outline:"none", background:"#faf7f2" }} onFocus={e=>e.target.style.borderColor="#f59e0b"} onBlur={e=>e.target.style.borderColor="#e7e5e4"} />
                </div>
                <div>
                  ...
                  <select value={form.with} onChange={e=>setForm(f=>({...f,with:e.target.value}))} style={{ width:"100%", border:"1.5px solid #e7e5e4", borderRadius:12, padding:"13px 16px", fontSize:15, color:form.with?"#1c1917":"#a8a29e", outline:"none", background:"#faf7f2", appearance:"none" }}>
              ...
              <div style={{ marginBottom:20 }}>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                  {INTERESTS.map(item => {
                    ...
                    return <button key={item} className="pill" onClick={()=>toggleInterest(item)} style={{ padding:"7px 14px", borderRadius:20, fontSize:13, fontFamily:"'DM Sans',sans-serif", fontWeight:500, background:active?"#292524":"#f5f5f4", color:active?"#faf7f2":"#78716c", border:active?"1.5px solid #292524":"1.5px solid #e7e5e4" }}>{item}</button>;
                  })}
                </div>
              </div>

              <div style={{ marginBottom:20 }}>
                <label style={{ display:"block", fontSize:13, color:"#78716c", marginBottom:8, fontFamily:"'DM Sans',sans-serif", fontWeight:600 }}>🍽️ Favorite cuisine</label>
                <input value={form.food} onChange={e=>setForm(f=>({...f,food:e.target.value}))} placeholder="Syrian, Italian, Japanese..." style={{ width:"100%", border:"1.5px solid #e7e5e4", borderRadius:12, padding:"13px 16px", fontSize:15, color:"#1c1917", outline:"none", background:"#faf7f2" }} onFocus={e=>e.target.style.borderColor="#f59e0b"} onBlur={e=>e.target.style.borderColor="#e7e5e4"} />
              </div>

              <div style={{ marginBottom:24 }}>
                <label style={{ display:"block", fontSize:13, color:"#78716c", marginBottom:8, fontFamily:"'DM Sans',sans-serif", fontWeight:600 }}>🎒 What do you need?</label>
                <input value={form.needs} onChange={e=>setForm(f=>({...f,needs:e.target.value}))} placeholder="Hotel, taxi, cinema..." style={{ width:"100%", border:"1.5px solid #e7e5e4", borderRadius:12, padding:"13px 16px", fontSize:15, color:"#1c1917", outline:"none", background:"#faf7f2" }} onFocus={e=>e.target.style.borderColor="#f59e0b"} onBlur={e=>e.target.style.borderColor="#e7e5e4"} />
              </div>

              <button onClick={generate} disabled={loading||!form.city.trim()} style={{ width:"100%", padding:"16px", background:loading?"#e7e5e4":"linear-gradient(135deg,#292524,#44403c)", border:"none", borderRadius:14, color:loading?"#a8a29e":"#faf7f2", fontSize:16, fontWeight:600, fontFamily:"'DM Sans',sans-serif", opacity:!form.city.trim()?0.5:1 }}>
        ...
        {step === "result" && result && (
          <div>
            <div className="fade" style={{ background:"#292524", borderRadius:20, padding:"28px 24px", marginBottom:16, position:"relative", overflow:"hidden", boxShadow:"0 8px 40px rgba(0,0,0,0.15)" }}>
              <div style={{ position:"absolute", top:-30, right:-30, width:120, height:120, borderRadius:"50%", background:"rgba(245,158,11,0.1)" }} />
